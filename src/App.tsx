...
export default function App() {
  ...
  const fetchReceipts = async () => {
    try {
      ...
      const filteredSales = response.data.list.map((item: { id: number; amount: number; datesent: string }) => ({
        id: item.id,
        amount: item.amount,
  ...
  const renderHeaderContent = () => {
    switch (activeTab) {
      case "Sales":
        return (
          <>
            <div css={css`position: relative; display: inline-block; margin-right: 1rem;`}>
      ...
      case "Incurred Costs":
        return (
          <>
            ...
            <button css={btnSecondary} onClick={() => setActiveTab("Expenses & Budget")}>
              Return Later
            </button>
          </>
        )
      ...
      <main css={mainContentStyles}>
        <header css={headerStyles}>
          ...
          <div css={css`display: flex; align-items: center; gap: 1rem;`}>
            ...
            {renderHeaderContent()}
            {activeTab === "Dashboard" && (
              <div css={css`position: relative; display: inline-block; cursor: pointer;`}>
                <span
                  className="bell-icon"
                  css={css`
                    font-size: 24px;
                    ...
                    animation: ${scheduledmeeting.length > 0 || budgetMonthsData.length > 0 || incurred ? "shake 0.5s ease-in-out infinite" : "none"};

...
function App() {
  ...
  const refreshData = async () => {
    try {
      // Refresh budget months data
      const budgetResponse = await axios.get(`${serverUrl}item/budgetList`)
      const budgetList = budgetResponse.data.list.map((item: { id: number; monthadded: string; status: number }) => ({
        id: item.id,
        name: item.monthadded,
        status: item.status,
      }))
      setBudgetMonthsData(budgetList)

      // Refresh incurred costs data
      const incurredResponse = await axios.get(`${serverUrl}incurredcost/list`)
      setIncurred(incurredResponse.data?.incurred ?? 0)
      setCreateddate(incurredResponse.data?.datecreated ?? "")

      // Refresh receipts data
      const receiptsResponse = await axios.get(`${serverUrl}item/receiptList`)
                ...
                {activeTab === "Dashboard" ? (
                  ...
                  >
                    ...
                    >
                      ...
                      {/* incurred */}
                      {/* budgetMonthsData */}
                      {scheduledmeeting.length > 0 || budgetMonthsData.length > 0 || incurred ? "\u{1F514}" : ""}
                    </span>
